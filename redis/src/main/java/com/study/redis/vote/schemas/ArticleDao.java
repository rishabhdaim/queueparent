...
public class ArticleDao {
    ...
    public static ArticleDao ofMap(final String id, final Map<String, String> value) {
        log.atInfo().log("Creating Article from map for %s", id);
        final var now = Instant.EPOCH.getEpochSecond();
        return value.isEmpty() ? EMPTY_ARTICLE_DAO : new ArticleDao(Try.of(() -> Long.parseLong(id.split(":")[1])).getOrElse(0L),
    ...
    private static final ArticleDao EMPTY_ARTICLE_DAO = ArticleDao.of(-1, "", "", "", -1, -1, -1);
}
